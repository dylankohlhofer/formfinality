...
| C05 | Push current development to the owner's GitHub | **Publication unblocked, 24 September.** The owner added Workflows permission to the existing fine-grained token. The non-forced push of `codex/automated-test-lab` succeeded and remote `56412b4` was verified; see project status for subsequent checkpoints. | Verify the remote SHA after each later commit/push. Never paste a token into chat, remove the workflow to bypass authorization, overwrite old `main` or remove `ethan` implicitly. |
| C06 | Make CI automatic and prove it runs | **Remote execution confirmed; completed result pending.** [Workflow](../.github/workflows/test-lab.yml) defines push/PR/manual runs, Swift/browser/audio/video/artifact checks and retained reports. [Run 36001701699](https://github.com/dylankohlhofer/formfinality/actions/runs/36001701699) started for `56412b4`; see project status for the latest run. | Inspect an actual completed remote run/artifacts. Missing `gh` did not prevent Git push or the authenticated read-only Actions API check. A started workflow or passing local suite is not a completed Actions pass. |
...
| A03 | Resolve audio stalls completely | **Recovery and two further defects repaired; original delivery cause still open.** Explicit playback errors now fail whole sentences, and the number harness no longer cuts off legally started late audio at four seconds. New reports retain browser/server/file-read timings and source copies. [Follow-up](sessions/audio-integrity-2026-09-24.md). | Use retained transport evidence to diagnose any renewed delay. Do not call the old OS/browser cause resolved or loosen production deadlines. Full-number negative and corruption controls remain mandatory. |
| A04 | Fix repetitive or contextually inappropriate feedback | **Policy fixes implemented; listening/comprehension open.** Shared budgets, obsolete-cue cancellation and less-encouragement preference exist. Changing set/result templates now decline static recordings and use complete local fallback; twenty-five shared JS/Swift clip cases protect routing. | Review flagged real speech at realistic pace and on physical output; approve wording/density with beginners. Zero analyzer candidates is not human approval. |
...
| Create private formfinality repo, move work toward main, push | Initial repository/early main exist. C05 records the now-verified development-branch publication; it does not imply a merge into main. |
