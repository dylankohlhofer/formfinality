...
4,023 checks, and `swift test` runs against the same JSON and passes 15/15 now that Xcode is
...
*Also done: "make the demo gate check a suite". It is `refGates` in `verify.mjs` — 176 checks
