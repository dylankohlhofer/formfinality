...
export async function report(dir, run) {
  ...
  await writeFile(resolve(dir, 'index.html'), `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>FormFinder test review</title>
    ...
    <h1>FormFinder — ${escape(run.status)}</h1><p>${escape(run.started)} · build ${escape(run.build)}<br>SHA-256 ${escape(run.buildHash)}</p>
    ...
    <details><summary>Optional on-device AI review</summary><p>Finalized repository-synthetic runs produce <a href="review-status.json">review status</a> and <a href="review-evidence.json">bounded evidence</a>. These artifacts are absent for private/video/external-input runs. AI is not run automatically. Opt-in local review can add <a href="review-import.json">validated review priorities</a>; selection still requires human judgement and never changes a test expectation or resolves a coverage gap.</p></details>
